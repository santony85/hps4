...
export class GlobalService {
  ...
  formatDateHeure(initialDate: string) {
    ...
    if(dateH.getMinutes() <= 9 && dateH.getHours() <= 9 ){
      return currentDate+" a "+('0'+dateH.getHours()+'H0'+dateH.getMinutes());
    }
    else if (dateH.getMinutes() <= 9){
      return currentDate+" a "+(dateH.getHours()+'H0'+dateH.getMinutes());
    }
    else if(dateH.getHours() <= 9){
      return currentDate+" a "+('0'+dateH.getHours()+'H'+dateH.getMinutes());
    }
    else{
  ...
  formatdateByNbjour(initialDate: string){
    ...
    if(date_now.getMinutes() <= 9 && date_now.getHours() <= 9 ){
      return txtday+" à "+('0'+date_now.getHours()+'H0'+date_now.getMinutes())+txtmin;
    }
    else if (date_now.getMinutes() <= 9){
      return txtday+" à "+(date_now.getHours()+'H0'+date_now.getMinutes())+txtmin;
    }
    else if(date_now.getHours() <= 9){
      return txtday+" à "+('0'+date_now.getHours()+'H'+date_now.getMinutes())+txtmin;
    }
    else{
      return txtday+" à "+date_now.getHours()+'H'+date_now.getMinutes()+txtmin;
    }
  ...
  public getRdv(){
    ...
    this.storage.get('rdv').then(data => {
      ...
      for (var i = 0; i < data['data'].length; i++) {
        ...
        data['data'][i].dateDiff = this.diff_days(new Date(), new Date(data['data'][i].dateRdv));
  ...
  public getNbRdv(){
    return new Promise(resolve => {
      ...
      this.getRdv().then(data => {
        for (var i = 0; i < data['data'].length; i++) {
          if (data['data'][i].dateDiff == 0 && data['data'][i].status=="rdv" && data['data'][i].issue=="") {

            if (data['data'][i].issuerdv == "ABS")data['data'][i].color=0; 
            else if (data['data'][i].issuerdv == "CONFIRME")data['data'][i].color=1; 
            else if (data['data'][i].issuerdv == "REFUS")data['data'][i].color=5; 
            else {
    ...
    return new Promise(resolve => {
      ...
      this.getRdv().then(data => {
        for (var i = 0; i < data['data'].length; i++) {
          ...
          if (dtdiff > 0   && data['data'][i].status=="rdv" && data['data'][i].issuerdv=="") {
  ...
  public updateRdvFake() {
    this.storage.get('rdv').then(data => {
      for (var i = 0; i < data['data'].length; i++) {
        if(data['data'][i].heureRdv)data['data'][i].heureRDV=data['data'][i].heureRdv;
      }
